...
test("calculateQuizResult returns a strong ENFJ leaning for agreement-heavy matching answers", async () => {
  const { default: questions } = await import("../data/questions.json");

  const result = calculateQuizResult(questions, createAnswerMap("stronglyAgree"));
...
test("calculateQuizResult flips letters when reverse-coded answers dominate", async () => {
  ...
  const result = calculateQuizResult(questions, createAnswerMap("stronglyDisagree"));

  assert.equal(result.type, "ISTP");
...
test("reliability drops when paired questions are answered inconsistently", async () => {
  ...
  const answers = Object.fromEntries(
    questions.map((question, index) => [
      question.id,
      index % 2 === 0 ? "stronglyAgree" : "stronglyDisagree",
    ]),
